...
public class SingleTransferWorkerTest extends AbstractDropboxTest {
    ...
    public void testUpload() throws Exception {
        ...
        assertTrue(new CryptoFindFeature(session, new DropboxFindFeature(session), cryptomator).find(dir1, new DisabledListProgressListener()));
        assertEquals(content.length, new CryptoAttributesFeature(session, new DefaultAttributesFinderFeature(session), cryptomator).find(file1, new DisabledListProgressListener()).getSize());
        {
            ...
            final InputStream in = new CryptoReadFeature(session, new DropboxReadFeature(session), cryptomator).read(file1, new TransferStatus().length(content.length), new DisabledConnectionCallback());
        ...
        new CryptoDeleteFeature(session, new DropboxDeleteFeature(session), cryptomator).delete(Arrays.asList(file1, file2, dir1, vault), new DisabledLoginCallback(), new Delete.DisabledCallback());
        localFile1.delete();
        localFile2.delete();
