...
public class SFTPWriteFeatureTest extends AbstractSFTPTest {
    ...
    public void testWriteThrottled() throws Exception {
        ...
        final OutputStream out = new ThrottledOutputStream(new SFTPWriteFeature(session).write(test, status, ConnectionCallback.noop),
    ...
    public void testWrite() throws Exception {
        ...
        new SFTPDeleteFeature(session).delete(Arrays.asList(test, folder), LoginCallback.noop, new Delete.DisabledCallback());
    }
    ...
    public void testWriteSymlink() throws Exception {
        ...
        final OutputStream out = new SFTPWriteFeature(session).write(symlink, status, ConnectionCallback.noop);
        new StreamCopier(status, status).transfer(new ByteArrayInputStream(content), out);
        out.close();
        {
            final ByteArrayOutputStream buffer = new ByteArrayOutputStream(content.length);
        ...
        new SFTPDeleteFeature(session).delete(Arrays.asList(target, symlink), LoginCallback.noop, new Delete.DisabledCallback());
    }
    ...
    public void testWriteNotFound() throws Exception {
        ...
        new SFTPWriteFeature(session).write(test, new TransferStatus(), ConnectionCallback.noop);
    }
