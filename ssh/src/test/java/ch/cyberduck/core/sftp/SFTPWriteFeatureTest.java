...
public class SFTPWriteFeatureTest {
    ...
    public void testWriteThrottled() throws Exception {
        ...
        assertEquals(content.length, new SFTPWriteFeature(session).append(test, status.getLength(), PathCache.empty()).size, 0L);
        {
            ...
            final InputStream in = new SFTPReadFeature(session).read(test, new TransferStatus().length(content.length), new DisabledConnectionCallback(), new DisabledPasswordCallback());
            new StreamCopier(status, status).transfer(in, buffer);
            assertArrayEquals(content, buffer.toByteArray());
        }
        {
            final ByteArrayOutputStream buffer = new ByteArrayOutputStream(content.length - 1);
            final InputStream in = new SFTPReadFeature(session).read(test, new TransferStatus().append(true).skip(1L), new DisabledConnectionCallback(), new DisabledPasswordCallback());
            new StreamCopier(status, status).transfer(in, buffer);
    ...
    public void testWriteSymlink() throws Exception {
        ...
        {
            final ByteArrayOutputStream buffer = new ByteArrayOutputStream(content.length);
            final InputStream in = new SFTPReadFeature(session).read(symlink, new TransferStatus().length(content.length), new DisabledConnectionCallback(), new DisabledPasswordCallback());
            new StreamCopier(status, status).transfer(in, buffer);
            assertArrayEquals(content, buffer.toByteArray());
        }
        {
            final byte[] buffer = new byte[0];
            final InputStream in = new SFTPReadFeature(session).read(target, new TransferStatus(), new DisabledConnectionCallback(), new DisabledPasswordCallback());
    ...
    public void testWriteContentRange() throws Exception {
        ...
        IOUtils.copy(new SFTPReadFeature(session).read(test, new TransferStatus().length(content.length), new DisabledConnectionCallback(), new DisabledPasswordCallback()), out);
