...
public class SMBListService implements ListService {
    ...
    public AttributedList<Path> list(final Path directory, final ListProgressListener listener) throws BackgroundException {
        ...
        String dst = directory.getAbsolute();
        if(dst.charAt(0) == '/') {
            dst = dst.substring(1).replace("/", "\\");
        }
        ...
        try {
            ...
            if(e.getStatusCode() == 3221226071L) {
                throw new UnsupportedException("This folder is a DFS Share, which is currently not supported");
            } else {
