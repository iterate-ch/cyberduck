...
public class DefaultTemporaryFileServiceTest {
    ...
    public void testCreateFile() {
        ...
        assertEquals(String.format("%s%su%sp%s-196513505%sf", temp, s, s, s, s),
    ...
    public void testVersion() {
        ...
        final String s = FileSystems.getDefault().getSeparator();
        {
            ...
            assertEquals(String.format("%s%su%sp%s1234944045%sf", temp, s, s, s, s),
                    new DefaultTemporaryFileService().create("u", file).getAbsolute());
        }
        {
            ...
            file.attributes().setRegion("region");
            file.attributes().setVersionId("2");
            assertEquals(String.format("%s%su%s1234944045%sp", temp, s, s, s),
                    new DefaultTemporaryFileService().create("u", file).getAbsolute());
        }
    }
    ...
    public void testCreateContainer() {
        ...
        file.attributes().setRegion("region");
        assertEquals(String.format("%s%su%s-196513505%scontainer", temp, s, s, s),
                new DefaultTemporaryFileService().create("u", file).getAbsolute());
    }
    ...
    public void testPathTooLong() {
        ...
        assertNotEquals(String.format("%s/%s%s/1234944045/%s", temp, "UID", testPathDirectory, testPathFile).replace('/', File.separatorChar), localFile);
        assertEquals(String.format("%s/%s/%s/1234944045/%s", temp, "UID", testPathMD5, testPathFile).replace('/', File.separatorChar), localFile);
    }
    ...
    public void testPathNotTooLong() {
        ...
        final Local local = new DefaultTemporaryFileService().create("UID", file);
        final String localFile = local.getAbsolute();
        assertEquals(String.format("%s/%s%s/1234944045/%s", temp, "UID", testPathDirectory, testPathFile).replace('/', File.separatorChar), localFile);
        assertNotEquals(String.format("%s/%s%s/1234944045/%s", temp, "UID", testPathMD5, testPathFile).replace('/', File.separatorChar), localFile);
    }
    ...
    public void testTemporaryPath() {
        ...
        assertEquals("1206314894", local.getParent().getName());
