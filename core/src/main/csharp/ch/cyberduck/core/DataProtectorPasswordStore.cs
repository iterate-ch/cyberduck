...
namespace Ch.Cyberduck.Core
{
    public class DataProtectorPasswordStore : HostPasswordStore
    {
        ...
        {
            Host host = new Host(ProtocolFactory.global.forScheme(scheme.name()), hostName, port);
        ...
        {
            Host host = new Host(ProtocolFactory.global.forScheme(Scheme.ftp.name()), hostName);
            host.getCredentials().setUsername(user);
            return getPassword(host);
        }
