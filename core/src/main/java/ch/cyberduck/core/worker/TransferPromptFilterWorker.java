    ...
    public Map<TransferItem, TransferStatus> run(final Session<?> session) throws BackgroundException {
        ...
        for(Map.Entry<CacheReference<TransferItem>, AttributedList<TransferItem>> entry : cache.asMap().entrySet()) {
            ...
            for(TransferItem file : list) {
                ...
                final boolean accept = filter.accept(file.remote, file.local, new TransferStatus().setExists(true), listener);
                status.put(file, filter.prepare(file.remote, file.local, new TransferStatus().setExists(true), listener)
                        .setRejected(!accept));
            }
        }
        return status;
