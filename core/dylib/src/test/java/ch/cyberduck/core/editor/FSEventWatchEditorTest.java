...
public class FSEventWatchEditorTest {
    ...
    public void testTemporaryPath() throws Exception {
        ...
        final FSEventWatchEditor editor = new FSEventWatchEditor(new Application("com.apple.TextEdit", null),
                new SingleSessionPool(new TestLoginConnectionService(), new NullSession(new Host(new TestProtocol(), "h")), PathCache.empty(), new DisabledPasswordCallback()), path, new DisabledListProgressListener());
    ...
    public void testSymlinkTarget() throws Exception {
        ...
        final FSEventWatchEditor editor = new FSEventWatchEditor(new Application("com.apple.TextEdit", null),
                new SingleSessionPool(new TestLoginConnectionService(), new NullSession(new Host(new TestProtocol(), "h")), PathCache.empty(), new DisabledPasswordCallback()), file, new DisabledListProgressListener());
