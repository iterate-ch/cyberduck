...
public class SwiftSegmentServiceTest extends AbstractSwiftTest {
    ...
    public void testGetSegmentsDirectory() {
        ...
        assertEquals("/test.cyberduck.ch/.prefix/" + key + "/3", service.getSegmentsDirectory(new Path(container, key, EnumSet.of(Path.Type.file))).getAbsolute());
        final Path directory = new Path(container, "dir", EnumSet.of(Path.Type.directory));
        assertEquals("/test.cyberduck.ch/dir/.prefix/" + key + "/3", service.getSegmentsDirectory(new Path(directory, key, EnumSet.of(Path.Type.file))).getAbsolute());
    }
    ...
    public void testGetSegmentName() {
        ...
        assertEquals("/test.cyberduck.ch/dir/.prefix/sub/name/1/00000001", service.getSegment(new Path(directory, key, EnumSet.of(Path.Type.file)), 1).getAbsolute());
    }
}
