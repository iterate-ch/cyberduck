...
public class CopyWorkerTest {
    ...
    public void testCopyFile() throws Exception {
        ...
        worker.run(session);
        assertTrue(new CryptoFindFeature(session, new S3FindFeature(session), cryptomator).find(source, new DisabledListProgressListener()));
        assertTrue(new CryptoFindFeature(session, new S3FindFeature(session), cryptomator).find(target, new DisabledListProgressListener()));
    ...
    public void testCopyFolder() throws Exception {
        ...
        assertTrue(new CryptoFindFeature(session, new S3FindFeature(session), cryptomator).find(file, new DisabledListProgressListener()));
        ...
        assertTrue(new CryptoFindFeature(session, new S3FindFeature(session), cryptomator).find(folder, new DisabledListProgressListener()));
    ...
    public void testCopyFileIntoVault() throws Exception {
        ...
        assertTrue(new S3FindFeature(session).find(cleartextFile, new DisabledListProgressListener()));
    ...
    public void testCopyDirectoryIntoVault() throws Exception {
        ...
        assertTrue(new S3FindFeature(session).find(cleartextFolder, new DisabledListProgressListener()));
        assertTrue(new S3FindFeature(session).find(cleartextFile, new DisabledListProgressListener()));
        final CryptoVault cryptomator = new CryptoVault(vault);
        cryptomator.create(session, null, new VaultCredentials("test"), new DisabledPasswordStore());
        final DefaultVaultRegistry registry = new DefaultVaultRegistry(new DisabledPasswordStore(), new DisabledPasswordCallback(), cryptomator);
