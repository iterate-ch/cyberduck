...
public class S3SearchFeatureTest extends AbstractS3Test {
    ...
    public void testSearchInBucket() throws Exception {
        ...
        final Path file = new S3TouchFeature(session).touch(new S3WriteFeature(session), new Path(bucket, name, EnumSet.of(Path.Type.file)), new TransferStatus());
    ...
    public void testSearchInDirectory() throws Exception {
        ...
        session.getFeature(Touch.class).touch(new S3WriteFeature(session), file, new TransferStatus());
        final S3SearchFeature feature = new S3SearchFeature(session);
        ...
        final Path subdir = new S3DirectoryFeature(session).mkdir(new S3WriteFeature(session), new Path(bucket, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), new TransferStatus());
        ...
        final Path filesubdir = new S3TouchFeature(session).touch(new S3WriteFeature(session), new Path(subdir, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.file)), new TransferStatus());
        {
