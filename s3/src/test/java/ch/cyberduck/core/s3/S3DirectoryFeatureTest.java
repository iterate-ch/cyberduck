...
public class S3DirectoryFeatureTest extends AbstractS3Test {
        ...
        for(Location.Name region : session.getHost().getProtocol().getRegions()) {
            switch(region.getIdentifier()) {
                ...
                default:
                    ...
                    feature.mkdir(new S3WriteFeature(session), test, new TransferStatus().setRegion(region.getIdentifier()));
    ...
    @Test
    public void testCreateBucketAWS2SignaturePathStyle() throws Exception {
    ...
    @Test
    public void testCreateBucketAWS2Signature() throws Exception {
        final ProtocolFactory factory = new ProtocolFactory(new HashSet<>(Collections.singleton(new S3Protocol())));
        final Profile profile = new ProfilePlistReader(factory).read(
                this.getClass().getResourceAsStream("/S3 AWS2 Signature Version (HTTPS).cyberduckprofile"));
        final Host host = new Host(profile, profile.getDefaultHostname(), new Credentials(
    ...
    public void testCreateBucketInvalidName() throws Exception {
        ...
        assertFalse(new S3DirectoryFeature(session).isSupported(test.getParent(), test.getName()));
        assertTrue(new S3DirectoryFeature(virtualhost).isSupported(test.getParent(), test.getName()));
        final S3LocationFeature.S3Region region = new S3LocationFeature.S3Region("eu-west-2");
        test.attributes().setRegion(region.getIdentifier());
        new S3DirectoryFeature(session).mkdir(new S3WriteFeature(session), test, new TransferStatus().setRegion(region.getIdentifier()));
        assertTrue(new S3FindFeature(session).find(test));
        new S3DefaultDeleteFeature(session, acl).delete(Collections.singletonList(test), new DisabledLoginCallback(), new Delete.DisabledCallback());
    }
    ...
    public void testCreatePlaceholder() throws Exception {
        ...
        final S3AccessControlListFeature acl = new S3AccessControlListFeature(session);
        final Path test = new S3DirectoryFeature(session).mkdir(new S3WriteFeature(session), new Path(container, name, EnumSet.of(Path.Type.directory)), new TransferStatus());
    ...
    public void testCreatePlaceholderMinio() throws Exception {
        ...
        final Path test = new S3DirectoryFeature(session).mkdir(new S3WriteFeature(session), new Path(bucket, name, EnumSet.of(Path.Type.directory)), new TransferStatus());
        assertTrue(test.getType().contains(Path.Type.placeholder));
        assertTrue(new S3FindFeature(session).find(test));
        ...
        assertTrue(new S3VersionedObjectListService(session, acl).list(test, new DisabledListProgressListener()).isEmpty());
        assertTrue(new DefaultFindFeature(session).find(test));
        new S3DefaultDeleteFeature(session, acl).delete(Arrays.asList(test, bucket), new DisabledLoginCallback(), new Delete.DisabledCallback());
    }
    ...
    public void testCreatePlaceholderVersioningDelete() throws Exception {
        ...
        final S3AccessControlListFeature acl = new S3AccessControlListFeature(session);
        final Path test = new S3DirectoryFeature(session).mkdir(new S3WriteFeature(session), new Path(bucket, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), new TransferStatus());
        assertTrue(test.getType().contains(Path.Type.placeholder));
        assertTrue(new S3FindFeature(session).find(test));
    ...
    public void testCreatePlaceholderVersioningDeleteWithMarker() throws Exception {
        ...
        final Path test = new S3DirectoryFeature(session).mkdir(new S3WriteFeature(session), new Path(directory, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), new TransferStatus());
        assertTrue(test.getType().contains(Path.Type.placeholder));
        assertTrue(new S3FindFeature(session).find(test));
        assertTrue(new S3VersionedObjectListService(session, acl).list(directory, new DisabledListProgressListener()).contains(test));
        // Add delete marker
        new S3DefaultDeleteFeature(session, acl).delete(Collections.singletonList(new Path(test).withAttributes(PathAttributes.EMPTY)), new DisabledLoginCallback(), new Delete.DisabledCallback());
    ...
    public void testDirectoryDeleteWithVersioning() throws Exception {
        final Path bucket = new Path("versioning-test-eu-central-1-cyberduck", EnumSet.of(Path.Type.directory, Path.Type.volume));
        final S3AccessControlListFeature acl = new S3AccessControlListFeature(session);
        final Path parent = new S3DirectoryFeature(session).mkdir(new S3WriteFeature(session), new Path(bucket,
                new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), new TransferStatus());
        final Path test = new S3DirectoryFeature(session).mkdir(new S3WriteFeature(session), new Path(parent,
                new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), new TransferStatus());
        assertNotNull(test.attributes().getVersionId());
        assertTrue(test.isPlaceholder());
        // Only placeholder is found in list output with no version id set
        final Path placeholder = new S3ListService(session, acl, new S3VersioningFeature(session, acl)).list(parent, new DisabledListProgressListener()).find(new SimplePathPredicate(test));
        assertTrue(placeholder.isPlaceholder());
        assertTrue(new S3FindFeature(session).find(test));
        assertTrue(new DefaultFindFeature(session).find(test));
        // This will only cause a delete marker being added
        new S3DefaultDeleteFeature(session, acl).delete(Arrays.asList(new Path(test).withAttributes(PathAttributes.EMPTY), parent), new DisabledLoginCallback(), new Delete.DisabledCallback());
    ...
    public void testCreatePlaceholderVirtualHost() throws Exception {
        ...
        final Path test = new S3DirectoryFeature(virtualhost).mkdir(
                new S3WriteFeature(virtualhost), new Path(new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory, Path.Type.volume)), new TransferStatus());
    ...
    public void testBackslash() throws Exception {
        ...
        final Path test = new S3DirectoryFeature(session).mkdir(
                new S3WriteFeature(session), new Path(container, String.format("%s\\%s", new AlphanumericRandomStringService().random(),
                        new AlphanumericRandomStringService().random()), EnumSet.of(Path.Type.directory)), new TransferStatus());
        assertTrue(new S3FindFeature(session).find(test));
    ...
    public void testTouchUriEncoding() throws Exception {
        ...
        final Path directory = new S3DirectoryFeature(session).mkdir(
                new S3WriteFeature(session), new Path(container, String.format("%s-+*~@([", new AsciiRandomStringService().random()), EnumSet.of(Path.Type.file)), new TransferStatus());
        assertNull(directory.attributes().getVersionId());
        assertTrue(new S3FindFeature(session).find(directory));
        assertEquals(directory.attributes(), new S3AttributesFinderFeature(session).find(directory));
        final Path test = new S3TouchFeature(session).touch(
                new S3WriteFeature(session), new Path(directory, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.file)), new TransferStatus());
        assertTrue(new S3FindFeature(session).find(test));
        ...
        assertFalse(new S3FindFeature(session).find(directory));
    }
}
