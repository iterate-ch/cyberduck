...
public class S3DirectoryFeatureTest extends AbstractS3Test {
    ...
    public void testCreatePlaceholderMinio() throws Exception {
        ...
        final S3AccessControlListFeature acl = new S3AccessControlListFeature(session);
        final Path bucket = new S3PathStyleFallbackAdapter<>(host, session.getClient(), new BackgroundExceptionCallable<Path>() {
            @Override
        ...
        final Path test = new S3PathStyleFallbackAdapter<>(host, session.getClient(), new BackgroundExceptionCallable<Path>() {
            @Override
            public Path call() throws BackgroundException {
