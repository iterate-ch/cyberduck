...
public class S3ObjectListServiceTest extends AbstractS3Test {
    ...
    public void testList() throws Exception {
        ...
        assertNotSame(AttributedList.EMPTY, new S3ObjectListService(session).list(
    ...
    public void testListVirtualHostStyle() throws Exception {
        ...
        assertNotSame(AttributedList.EMPTY, new S3ObjectListService(virtualhost).list(
                bucket, new DisabledListProgressListener()));
    }
    ...
    public void testDirectory() throws Exception {
        ...
        final Path directory = new S3DirectoryFeature(session).mkdir(new S3WriteFeature(session), new Path(bucket, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), new TransferStatus());
        final S3ObjectListService feature = new S3ObjectListService(session);
    ...
    public void testListNotFoundFolder() throws Exception {
        ...
        final S3AccessControlListFeature acl = new S3AccessControlListFeature(session);
        assertThrows(NotfoundException.class, () -> new S3ObjectListService(session).list(new Path(container, name, EnumSet.of(Path.Type.directory)), new DisabledListProgressListener()));
        final Path file = new S3TouchFeature(session).touch(new S3WriteFeature(session), new Path(container, String.format("%s-", name), EnumSet.of(Path.Type.file)), new TransferStatus());
        assertThrows(NotfoundException.class, () -> new S3ObjectListService(session).list(new Path(container, name, EnumSet.of(Path.Type.directory)), new DisabledListProgressListener()));
    ...
    public void testListFile() throws Exception {
        ...
        final String name = new AlphanumericRandomStringService().random();
        final S3AccessControlListFeature acl = new S3AccessControlListFeature(session);
        final Path file = new S3TouchFeature(session).touch(new S3WriteFeature(session), new Path(container, name, EnumSet.of(Path.Type.file)), new TransferStatus());
        assertTrue(new S3ObjectListService(session).list(container, new DisabledListProgressListener()).contains(file));
        assertThrows(NotfoundException.class, () -> new S3ObjectListService(session).list(new Path(container, name, EnumSet.of(Path.Type.directory)), new DisabledListProgressListener()));
        new S3DefaultDeleteFeature(session, acl).delete(Collections.singletonList(file), new DisabledLoginCallback(), new Delete.DisabledCallback());
    }
    ...
    public void testListNotFoundFolderMinio() throws Exception {
        ...
        final Path directory = new S3DirectoryFeature(session).mkdir(
                new S3WriteFeature(session), new Path(new DefaultHomeFinderService(session).find(), new AsciiRandomStringService().random(), EnumSet.of(Path.Type.directory, Path.Type.volume)), new TransferStatus());
        try {
            new S3ObjectListService(session).list(new Path(directory, new AsciiRandomStringService(30).random(), EnumSet.of(Path.Type.directory)), new DisabledListProgressListener());
        }
        finally {
            new S3DefaultDeleteFeature(session, acl).delete(Collections.singletonList(directory), new DisabledLoginCallback(), new Delete.DisabledCallback());
        }
    ...
    public void testListEncodedCharacter() throws Exception {
        ...
        final Path placeholder = new S3TouchFeature(session).touch(
                new S3WriteFeature(session), new Path(container, String.format("^<%%%s", new AlphanumericRandomStringService().random()), EnumSet.of(Path.Type.file)), new TransferStatus());
    ...
    public void testListInvisibleCharacter() throws Exception {
        ...
        final S3AccessControlListFeature acl = new S3AccessControlListFeature(session);
        final Path placeholder = new S3TouchFeature(session).touch(
                new S3WriteFeature(session), new Path(container, String.format("test-\u001F-%s", new AlphanumericRandomStringService().random()), EnumSet.of(Path.Type.file)), new TransferStatus());
        assertTrue(new S3ObjectListService(session).list(container, new DisabledListProgressListener()).contains(placeholder));
        new S3DefaultDeleteFeature(session, acl).delete(Collections.singletonList(placeholder), new DisabledLoginCallback(), new Delete.DisabledCallback());
    }
    ...
    public void testListPlaceholder() throws Exception {
        ...
        final Path placeholder = new S3DirectoryFeature(session).mkdir(
                new S3WriteFeature(session), new Path(container, UUID.randomUUID().toString(), EnumSet.of(Path.Type.directory)), new TransferStatus());
    ...
    public void testListPlaceholderTilde() throws Exception {
        final Path container = new Path("test-eu-central-1-cyberduck", EnumSet.of(Path.Type.directory, Path.Type.volume));
        final S3AccessControlListFeature acl = new S3AccessControlListFeature(session);
        final Path placeholderTildeEnd = new S3DirectoryFeature(session).mkdir(
                new S3WriteFeature(session), new Path(container, String.format("%s~", new AlphanumericRandomStringService().random()), EnumSet.of(Path.Type.directory)), new TransferStatus());
        assertTrue(new S3ObjectListService(session).list(container, new DisabledListProgressListener()).contains(placeholderTildeEnd));
        final Path placeholderTildeStart = new S3DirectoryFeature(session).mkdir(
                new S3WriteFeature(session), new Path(container, String.format("~%s", new AlphanumericRandomStringService().random()), EnumSet.of(Path.Type.directory)), new TransferStatus());
        assertTrue(new S3ObjectListService(session).list(container, new DisabledListProgressListener()).contains(placeholderTildeStart));
        assertTrue(new S3ObjectListService(session).list(placeholderTildeEnd, new DisabledListProgressListener()).isEmpty());
        assertTrue(new S3ObjectListService(session).list(placeholderTildeStart, new DisabledListProgressListener()).isEmpty());
    ...
    public void testListPlaceholderAtSignSignatureAWS4() throws Exception {
        ...
        final S3AccessControlListFeature acl = new S3AccessControlListFeature(session);
        final Path placeholder = new S3DirectoryFeature(session).mkdir(
                new S3WriteFeature(session), new Path(container, String.format("%s@", UUID.randomUUID()), EnumSet.of(Path.Type.directory)), new TransferStatus());
        final AttributedList<Path> list = new S3ObjectListService(session).list(placeholder, new DisabledListProgressListener());
        assertTrue(list.isEmpty());
        new S3DefaultDeleteFeature(session, acl).delete(Collections.singletonList(placeholder), new DisabledLoginCallback(), new Delete.DisabledCallback());
    }
    ...
    public void testListFileDot() throws Exception {
        ...
        final S3AccessControlListFeature acl = new S3AccessControlListFeature(session);
        final Path file = new S3TouchFeature(session).touch(
                new S3WriteFeature(session), new Path(container, ".", EnumSet.of(Path.Type.file)), new TransferStatus());
        assertTrue(new S3ObjectListService(session).list(container, new DisabledListProgressListener()).contains(file));
        new S3DefaultDeleteFeature(session, acl).delete(Collections.singletonList(file), new DisabledLoginCallback(), new Delete.DisabledCallback());
    }
    ...
    public void testListPlaceholderDot() throws Exception {
        ...
        final S3AccessControlListFeature acl = new S3AccessControlListFeature(session);
        final Path placeholder = new S3DirectoryFeature(session).mkdir(
                new S3WriteFeature(session), new Path(container, ".", EnumSet.of(Path.Type.directory)), new TransferStatus());
        assertTrue(new S3ObjectListService(session).list(container, new DisabledListProgressListener()).contains(placeholder));
        new S3DefaultDeleteFeature(session, acl).delete(Collections.singletonList(placeholder), new DisabledLoginCallback(), new Delete.DisabledCallback());
    }
    ...
    public void testListPlaceholderPlusCharacter() throws Exception {
        ...
        final S3AccessControlListFeature acl = new S3AccessControlListFeature(session);
        final Path placeholder = new S3DirectoryFeature(session).mkdir(
                new S3WriteFeature(session), new Path(container, String.format("test+%s", new AlphanumericRandomStringService().random()), EnumSet.of(Path.Type.directory)), new TransferStatus());
        assertTrue(new S3ObjectListService(session).list(container, new DisabledListProgressListener()).contains(placeholder));
    ...
    public void testDetermineCorrectRegionFrom400Reply() throws Exception {
        ...
        feature.mkdir(new S3WriteFeature(session), bucket, new TransferStatus().setRegion("eu-central-1"));
        ...
        assertNotSame(AttributedList.emptyList(), new S3ObjectListService(session).list(bucket, new DisabledListProgressListener()));
        assertEquals("eu-central-1", cache.getRegionForBucketName(bucket.getName()));
    ...
    public void testListCommonPrefixSlashOnly() throws Exception {
        ...
        final S3AccessControlListFeature acl = new S3AccessControlListFeature(session);
        assertTrue(new S3ObjectListService(session).list(container, new DisabledListProgressListener()).isEmpty());
    }
}
