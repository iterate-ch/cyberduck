...
public class S3FindFeatureTest extends AbstractS3Test {
    ...
    public void testFindRoot() throws Exception {
        assertTrue(new S3FindFeature(session).find(Home.root()));
        assertTrue(new S3FindFeature(virtualhost).find(Home.root()));
    }
    ...
    public void testFindCommonPrefix() throws Exception {
        ...
        assertFalse(new S3FindFeature(session).find(new Path(test.getAbsolute(), EnumSet.of(Path.Type.directory))));
    ...
    public void testFindCommonPrefixWithVirtualHost() throws Exception {
        ...
        assertTrue(new S3FindFeature(virtualhost).find(container));
        final String prefix = new AlphanumericRandomStringService().random();
        final Path test = new S3TouchFeature(virtualhost).touch(
                new S3WriteFeature(virtualhost), new Path(new Path(container, prefix, EnumSet.of(Path.Type.directory)),
                        new AsciiRandomStringService().random(), EnumSet.of(Path.Type.file)), new TransferStatus());
        assertTrue(new S3FindFeature(virtualhost).find(test));
        assertFalse(new S3FindFeature(virtualhost).find(new Path(test.getAbsolute(), EnumSet.of(Path.Type.directory))));
        assertTrue(new S3FindFeature(virtualhost).find(new Path(container, prefix, EnumSet.of(Path.Type.directory))));
        assertTrue(new S3FindFeature(virtualhost).find(new Path(container, prefix, EnumSet.of(Path.Type.directory, Path.Type.placeholder))));
        assertTrue(new S3ObjectListService(virtualhost).list(new Path(container, prefix, EnumSet.of(Path.Type.directory)),
                new DisabledListProgressListener()).contains(test));
        ...
        assertFalse(new S3FindFeature(virtualhost).find(test));
        assertFalse(new S3FindFeature(virtualhost).find(new Path(container, prefix, EnumSet.of(Path.Type.directory))));
        final PathCache cache = new PathCache(1);
        final Path directory = new Path(container, prefix, EnumSet.of(Path.Type.directory, Path.Type.placeholder));
        assertFalse(new CachingFindFeature(virtualhost, cache).find(directory));
        assertTrue(cache.isCached(directory.getParent()));
        assertFalse(new S3FindFeature(virtualhost).find(new Path(container, prefix, EnumSet.of(Path.Type.directory, Path.Type.placeholder))));
    }
}
