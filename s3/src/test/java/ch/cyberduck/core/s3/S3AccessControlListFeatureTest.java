...
public class S3AccessControlListFeatureTest extends AbstractS3Test {
    ...
    public void testReadWithDelimiter() throws Exception {
        final Path container = new Path("test-eu-central-1-cyberduck", EnumSet.of(Path.Type.directory, Path.Type.volume));
        final Path test = new S3TouchFeature(session).touch(new S3WriteFeature(session), new Path(new Path(container, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.file)), new TransferStatus());
        final S3AccessControlListFeature f = new S3AccessControlListFeature(session);
    ...
    public void testReadDirectoryPlaceholder() throws Exception {
        ...
        final Path placeholder = new S3DirectoryFeature(session).mkdir(new S3WriteFeature(session), new Path(container, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), new TransferStatus());
