...
public class S3AccessControlListFeatureTest extends AbstractS3Test {
    ...
    public void testWriteVirtualHostBucket() throws Exception {
        ...
        new S3TouchFeature(virtualhost, new S3AccessControlListFeature(session)).touch(test, new TransferStatus().withLength(0L));
    ...
    public void testReadWithDelimiter() throws Exception {
        final Path container = new Path("test-eu-central-1-cyberduck", EnumSet.of(Path.Type.directory, Path.Type.volume));
        final Path test = new S3TouchFeature(session, new S3AccessControlListFeature(session)).touch(new Path(
                new Path(container, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.file)), new TransferStatus().withLength(0L));
        final S3AccessControlListFeature f = new S3AccessControlListFeature(session);
    ...
    public void testReadDirectoryPlaceholder() throws Exception {
        ...
        final Path placeholder = new S3DirectoryFeature(session, new S3WriteFeature(session, acl), acl).mkdir(new Path(container, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)),
    ...
    public void testReadVersioned() throws Exception {
        ...
        final Path test = new S3TouchFeature(session, new S3AccessControlListFeature(session)).touch(new Path(container, new AsciiRandomStringService().random(), EnumSet.of(Path.Type.file)),
                new TransferStatus().withLength(0L));
