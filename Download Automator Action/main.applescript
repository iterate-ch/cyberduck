	...
	if input is not {} then
		if the class of the input is list then
			...
			tell application "Cyberduck"
				...
				tell (theBrowser)
					connect to theServer with port thePort with protocol theProtocol as user theUser with password thePassword
					change folder thePath
					repeat with i in input
						download file (i as string)
					end repeat
					disconnect
				end tell
			end tell
