...
public class AzureWriteFeatureTest extends AbstractAzureTest {
    ...
    public void testWriteOverrideAppendBlob() throws Exception {
        ...
        assertEquals(content.length, new AzureWriteFeature(session).append(test, status.getLength(), PathCache.empty()).size, 0L);
        ...
        final OutputStream overwrite = new AzureWriteFeature(session).write(test, new TransferStatus().exists(true)
    ...
    public void testWriteOverrideBlockBlob() throws Exception {
        ...
        assertEquals("text/plain", metadata.get("Content-Type"));
        assertEquals("public,max-age=86400", metadata.get("Cache-Control"));
        final Write.Append append = new AzureWriteFeature(session).append(test, status.getLength(), PathCache.empty());
