...
public class AzureListServiceTest extends AbstractAzureTest {
    ...
    public void testListCryptomator() throws Exception {
        ...
        new CryptoTouchFeature<Void>(session, new DefaultTouchFeature<Void>(new AzureWriteFeature(session, null), new AzureAttributesFinderFeature(session, null)), new AzureWriteFeature(session, null), cryptomator).touch(test, new TransferStatus());
