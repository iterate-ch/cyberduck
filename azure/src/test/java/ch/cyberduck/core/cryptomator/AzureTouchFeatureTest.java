...
public class AzureTouchFeatureTest extends AbstractAzureTest {
    ...
    public void testTouchLongFilenameEncrypted() throws Exception {
        ...
        new CryptoTouchFeature<>(session, new AzureTouchFeature(session), new AzureWriteFeature(session), cryptomator).touch(test, new TransferStatus());
    ...
    public void testTouchLongFilenameEncryptedDefaultFeature() throws Exception {
        ...
        session.withRegistry(new DefaultVaultRegistry(new DisabledPasswordStore(), new DisabledPasswordCallback(), cryptomator));
        final Path test = new Path(vault, new RandomStringGenerator.Builder().build().generate(130), EnumSet.of(Path.Type.file));
        new CryptoTouchFeature<>(session, new DefaultTouchFeature<>(new DefaultUploadFeature<>(new AzureWriteFeature(session)), new AzureAttributesFinderFeature(session)), new AzureWriteFeature(session), cryptomator).touch(test, new TransferStatus());
