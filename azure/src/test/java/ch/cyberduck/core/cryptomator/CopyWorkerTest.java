...
public class CopyWorkerTest extends AbstractAzureTest {
    ...
    public void testCopyToDifferentFolderCryptomator() throws Exception {
        ...
        assertTrue(new CryptoFindFeature(session, new DefaultFindFeature(session), cryptomator).find(source));
        cryptomator.getFeature(session, Directory.class, new AzureDirectoryFeature(session, null)).mkdir(targetFolder, null, new TransferStatus());
    ...
    public void testCopyFolder() throws Exception {
        ...
        final DefaultVaultRegistry registry = new DefaultVaultRegistry(new DisabledPasswordStore(), new DisabledPasswordCallback(), cryptomator);
        session.withRegistry(registry);
        cryptomator.getFeature(session, Directory.class, new AzureDirectoryFeature(session, null)).mkdir(folder, null, new TransferStatus());
    ...
    public void testCopyFileIntoVault() throws Exception {
        ...
        final DefaultVaultRegistry registry = new DefaultVaultRegistry(new DisabledPasswordStore(), new DisabledPasswordCallback(), cryptomator);
        session.withRegistry(registry);
        cryptomator.getFeature(session, Directory.class, new AzureDirectoryFeature(session, null)).mkdir(encryptedFolder, null, new TransferStatus());
