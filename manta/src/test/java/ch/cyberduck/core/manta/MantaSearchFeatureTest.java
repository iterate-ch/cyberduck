...
public class MantaSearchFeatureTest extends AbstractMantaTest {
    ...
    public void testSearch() throws Exception {
        ...
        new MantaDirectoryFeature(session).mkdir(directory, null, new TransferStatus());
        final Path file = new Path(directory, name, EnumSet.of(Path.Type.file));
        new MantaTouchFeature(session).touch(file, new TransferStatus());
        final MantaSearchFeature feature = new MantaSearchFeature(session);
        ...
        new MantaDeleteFeature(session).delete(Arrays.asList(file), new DisabledLoginCallback(), new Delete.DisabledCallback());
    }
}
