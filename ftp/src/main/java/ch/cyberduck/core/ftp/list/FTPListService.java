...
public class FTPListService implements ListService {
    ...
    public FTPListService(final FTPSession session, final String system, final TimeZone zone) {
        ...
        if(Preferences.toBoolean(session.getHost().getProperty("ftp.command.stat"))) {
        ...
        if(Preferences.toBoolean(session.getHost().getProperty("ftp.command.mlsd"))) {
            this.implementations.put(Command.mlsd, new FTPMlsdListService(session));
        }
        if(Preferences.toBoolean(session.getHost().getProperty("ftp.command.lista"))) {
