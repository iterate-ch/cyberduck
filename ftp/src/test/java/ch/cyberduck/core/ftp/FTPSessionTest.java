...
//        assertFalse(session.isSecured());
    ...
    public void testConnectionTlsUpgrade() throws Exception {
        ...
        final FTPSession session = new FTPSession(host) {
            @Override
            public void login(final Proxy proxy, final HostPasswordStore keychain, final LoginCallback login, CancelCallback cancel) throws BackgroundException {
                assertEquals(Session.State.open, this.getState());
                super.login(proxy, keychain, login, cancel);
