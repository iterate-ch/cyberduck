...
public class CopyWorkerTest {
    ...
    public void testCopyFile() throws Exception {
        ...
        final CopyWorker worker = new CopyWorker(Collections.singletonMap(source, target), new TestSessionPool(copySession), new DisabledProgressListener());
        worker.run(session);
        assertTrue(new DefaultFindFeature(session).find(source));
    ...
    public void testCopyFileToDirectory() throws Exception {
        ...
        copySession.open(new DisabledHostKeyCallback());
        copySession.login(new DisabledPasswordStore(), new DisabledLoginCallback(), new DisabledCancelCallback(), PathCache.empty());
        final CopyWorker worker = new CopyWorker(Collections.singletonMap(sourceFile, targetFile), new TestSessionPool(copySession), new DisabledProgressListener());
        worker.run(session);
        assertTrue(new DefaultFindFeature(session).find(sourceFile));
    ...
    public void testCopyDirectory() throws Exception {
        ...
        copySession.open(new DisabledHostKeyCallback());
        copySession.login(new DisabledPasswordStore(), new DisabledLoginCallback(), new DisabledCancelCallback(), PathCache.empty());
        final CopyWorker worker = new CopyWorker(Collections.singletonMap(folder, targetFolder), new TestSessionPool(copySession), new DisabledProgressListener());
        worker.run(session);
        assertTrue(new DefaultFindFeature(session).find(targetFolder));
        assertTrue(new DefaultFindFeature(session).find(targetFile));
        assertTrue(new DefaultFindFeature(session).find(folder));
