...
public class MoveWorkerTest extends AbstractDriveTest {
    ...
    public void testMoveFolder() throws Exception {
        ...
        new MoveWorker(Collections.singletonMap(
                new Path(folder, filename, EnumSet.of(Path.Type.file)),
                new Path(folder, filenameRenamed, EnumSet.of(Path.Type.file))), new SessionPool.SingleSessionPool(session), PathCache.empty(), ProgressListener.noop, new DisabledLoginCallback()).run(session);
    ...
    public void testMoveDirectoryIntoVault() throws Exception {
        ...
        final CopyWorker worker = new CopyWorker(Collections.singletonMap(clearFolder, encryptedFolder), new SessionPool.SingleSessionPool(session, registry), PathCache.empty(), ProgressListener.noop, new DisabledLoginCallback());
        worker.run(session);
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(encryptedFolder));
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(encryptedFile));
        cryptomator.getFeature(session, Delete.class, new DriveDeleteFeature(session, fileid)).delete(Arrays.asList(encryptedFile, encryptedFolder, vault), new DisabledLoginCallback(), new Delete.DisabledCallback());
    ...
    public void testMoveFileOutsideVault() throws Exception {
        ...
        final MoveWorker worker = new MoveWorker(Collections.singletonMap(encryptedFile, fileRenamed), new SessionPool.SingleSessionPool(session, registry), PathCache.empty(), ProgressListener.noop, new DisabledLoginCallback());
        worker.run(session);
    ...
    public void testMoveDirectoryOutsideVault() throws Exception {
        ...
        final MoveWorker worker = new MoveWorker(Collections.singletonMap(encryptedFolder, directoryRenamed), new SessionPool.SingleSessionPool(session, registry), PathCache.empty(), ProgressListener.noop, new DisabledLoginCallback());
        worker.run(session);
