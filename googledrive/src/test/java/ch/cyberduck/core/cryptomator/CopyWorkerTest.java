...
public class CopyWorkerTest extends AbstractDriveTest {
    ...
    public void testCopyFile() throws Exception {
        ...
        worker.run(session);
        assertTrue(new CryptoFindV6Feature(session, new DriveFindFeature(session, fileid), cryptomator).find(source));
        assertTrue(new CryptoFindV6Feature(session, new DriveFindFeature(session, fileid), cryptomator).find(target));
    ...
    public void testCopyToDifferentFolderCryptomator() throws Exception {
        ...
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(source));
        cryptomator.getFeature(session, Directory.class, new DriveDirectoryFeature(session, fileid)).mkdir(targetFolder, new TransferStatus());
    ...
    public void testCopyFolder() throws Exception {
        ...
        session.withRegistry(registry);
        final DriveFileIdProvider fileid = new DriveFileIdProvider(session);
        cryptomator.getFeature(session, Directory.class, new DriveDirectoryFeature(session, fileid)).mkdir(folder, new TransferStatus());
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(folder));
        new CryptoTouchFeature<>(session, new DefaultTouchFeature<>(new DriveWriteFeature(session, fileid)), new DriveWriteFeature(session, fileid), cryptomator).touch(file, new TransferStatus());
        ...
        assertTrue(new CryptoFindV6Feature(session, new DriveFindFeature(session, fileid), cryptomator).find(file));
        assertTrue(new CryptoFindV6Feature(session, new DriveFindFeature(session, fileid), cryptomator).find(fileRenamed));
        // copy folder
        ...
        assertTrue(new CryptoFindV6Feature(session, new DriveFindFeature(session, fileid), cryptomator).find(folder));
        assertTrue(new CryptoFindV6Feature(session, new DriveFindFeature(session, fileid), cryptomator).find(folderRenamed));
    ...
    public void testCopyFileIntoVault() throws Exception {
        ...
        final DefaultVaultRegistry registry = new DefaultVaultRegistry(new DisabledPasswordStore(), new DisabledPasswordCallback(), cryptomator);
        session.withRegistry(registry);
        cryptomator.getFeature(session, Directory.class, new DriveDirectoryFeature(session, fileid)).mkdir(encryptedFolder, new TransferStatus());
        ...
        worker.run(session);
        assertTrue(new DriveFindFeature(session, fileid).find(cleartextFile));
        assertTrue(new CryptoFindV6Feature(session, new DriveFindFeature(session, fileid), cryptomator).find(encryptedFile));
        registry.clear();
        new DeleteWorker(new DisabledLoginCallback(), Collections.singletonList(vault), new DisabledProgressListener()).run(session);
    }
    ...
    public void testCopyDirectoryIntoVault() throws Exception {
        ...
        final DriveFileIdProvider fileid = new DriveFileIdProvider(session);
        new DriveDirectoryFeature(session, fileid).mkdir(cleartextFolder, new TransferStatus());
        new DriveTouchFeature(session, fileid).touch(cleartextFile, new TransferStatus());
    ...
    public void testCopyFileOutsideVault() throws Exception {
        ...
        cryptomator.getFeature(session, Directory.class, new DriveDirectoryFeature(session, fileid)).mkdir(encryptedFolder, new TransferStatus());
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(encryptedFolder));
        new CryptoTouchFeature<>(session, new DefaultTouchFeature<>(new DriveWriteFeature(session, fileid)), new DriveWriteFeature(session, fileid), cryptomator).touch(encryptedFile, new TransferStatus());
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(encryptedFile));
    ...
    public void testCopyDirectoryOutsideVault() throws Exception {
        ...
        worker.run(session);
        assertTrue(new CryptoFindV6Feature(session, new DriveFindFeature(session, fileid), cryptomator).find(encryptedFolder));
        assertTrue(new CryptoFindV6Feature(session, new DriveFindFeature(session, fileid), cryptomator).find(encryptedFile));
        assertTrue(new DriveFindFeature(session, fileid).find(cleartextFolder));
