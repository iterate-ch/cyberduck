...
public class CopyWorkerTest extends AbstractDriveTest {
    ...
    public void testCopyFileOutsideVault() throws Exception {
        ...
        worker.run(session);
        assertTrue(new CryptoFindFeature(session, new DriveFindFeature(session, fileid), cryptomator).withCache(cache).find(encryptedFile));
        assertTrue(new DriveFindFeature(session, fileid).withCache(cache).find(cleartextFile));
        registry.clear();
