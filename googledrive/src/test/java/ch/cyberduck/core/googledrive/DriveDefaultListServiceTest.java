...
public class DriveDefaultListServiceTest extends AbstractDriveTest {
    ...
    public void testListSharedDriveFolder() throws Exception {
        ...
        final Path directory = new DriveDirectoryFeature(session, fileid).mkdir(
                new DriveWriteFeature(session, fileid), new Path(new Path(SHARED_DRIVES_NAME, "iterate", EnumSet.of(Path.Type.directory)), new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), new TransferStatus());
        final Path file = new DriveTouchFeature(session, fileid).touch(new DriveWriteFeature(session, fileid), new Path(directory, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.file)), new TransferStatus());
        final AttributedList<Path> list = new DriveDefaultListService(session, fileid).list(directory, new DisabledListProgressListener());
        assertFalse(list.isEmpty());
        for(Path f : list) {
    ...
    public void testListMissingFileidOnFolder() throws Exception {
        ...
        final Path f2 = new DriveTouchFeature(session, fileid).touch(new DriveWriteFeature(session, fileid), new Path(directory, "aa", EnumSet.of(Path.Type.file)), new TransferStatus());
        final Path f1 = new DriveTouchFeature(session, fileid).touch(new DriveWriteFeature(session, fileid), new Path(directory, "a", EnumSet.of(Path.Type.file)), new TransferStatus());
    ...
    public void testListLexicographicallyNumbers() throws Exception {
        ...
        final Path directory = new DriveDirectoryFeature(session, fileid).mkdir(
                new DriveWriteFeature(session, fileid), new Path(MYDRIVE_FOLDER, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), new TransferStatus());
        final Path f2 = new DriveTouchFeature(session, fileid).touch(new DriveWriteFeature(session, fileid), new Path(directory, "103", EnumSet.of(Path.Type.file)), new TransferStatus());
        final Path f1 = new DriveTouchFeature(session, fileid).touch(new DriveWriteFeature(session, fileid), new Path(directory, "101", EnumSet.of(Path.Type.file)), new TransferStatus());
        assertTrue(f1.getName().compareTo(f2.getName()) < 0);
        final AttributedList<Path> list = new DriveDefaultListService(session, fileid).list(directory, new DisabledListProgressListener());
        assertEquals(2, list.size());
        assertEquals(f1, list.get(0));
    ...
    public void testListFilenameSingleQuote() throws Exception {
        ...
        final Path directory = new DriveDirectoryFeature(session, fileid).mkdir(
                new DriveWriteFeature(session, fileid), new Path(MYDRIVE_FOLDER, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), new TransferStatus());
        final Path f1 = new DriveTouchFeature(session, fileid).touch(new DriveWriteFeature(session, fileid), new Path(directory, String.format("%s'", new AsciiRandomStringService().random()), EnumSet.of(Path.Type.file)), new TransferStatus());
        final AttributedList<Path> list = new DriveDefaultListService(session, fileid).list(directory, new DisabledListProgressListener());
    ...
    public void testFilenameColon() throws Exception {
        ...
        new DriveDeleteFeature(session, fileid).delete(Arrays.asList(file, folder), LoginCallback.noop, new Delete.DisabledCallback());
    }
    ...
    public void testSameFoldername() throws Exception {
        ...
        new DriveDirectoryFeature(session, provider).mkdir(new DriveWriteFeature(session, provider), parent, new TransferStatus());
        new DriveDirectoryFeature(session, provider).mkdir(new DriveWriteFeature(session, provider), folder, new TransferStatus());
        assertTrue(new DefaultFindFeature(session).find(folder));
        ...
        new DriveDeleteFeature(session, provider).delete(Collections.singletonList(parent), LoginCallback.noop, new Delete.DisabledCallback());
    }
