...
public class DistributionUrlProviderTest extends AbstractTestCase {
    ...
    public void testDownload() throws Exception {
        ...
        assertEquals("https://test.cyberduck.ch.s3.amazonaws.com/p/f",
                new DistributionUrlProvider(distribution).toUrl(
                        new Path("/test.cyberduck.ch/p/f", Path.FILE_TYPE)).find(DescriptiveUrl.Type.origin).getUrl());
    }
    ...
    public void testStreaming() throws Exception {
        ...
        assertEquals("rtmp://d1f6cbdjcbzyiu.cloudfront.net/cfx/st/p/f",
                new DistributionUrlProvider(distribution).toUrl(new Path("/test.cyberduck.ch/p/f", Path.FILE_TYPE)).find(DescriptiveUrl.Type.cdn).getUrl());
        assertEquals("https://test.cyberduck.ch.s3.amazonaws.com/p/f",
                new DistributionUrlProvider(distribution).toUrl(new Path("/test.cyberduck.ch/p/f", Path.FILE_TYPE)).find(DescriptiveUrl.Type.origin).getUrl());
    }
    ...
    public void testCustomOrigin() throws Exception {
        ...
        assertEquals("http://test.cyberduck.ch/p/f",
                new DistributionUrlProvider(distribution).toUrl(new Path("/p/f", Path.DIRECTORY_TYPE)).find(DescriptiveUrl.Type.origin).getUrl());
    }
