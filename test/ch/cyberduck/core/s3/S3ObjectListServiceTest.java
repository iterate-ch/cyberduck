...
public class S3ObjectListServiceTest extends AbstractTestCase {
    ...
    public void testListDefaultPath() throws Exception {
        ...
        assertFalse(list.isEmpty());
        assertTrue(list.contains(new Path("/dist.springframework.org/release/SWF", EnumSet.of(Path.Type.directory))));
        session.close();
    }
    ...
    public void testListVersioning() throws Exception {
        ...
        assertTrue(list.contains(new Path("/versioning.test.cyberduck.ch/test", EnumSet.of(Path.Type.file), att)));
