...
public class StoregateCopyFeatureTest extends AbstractStoregateTest {
    ...
    public void testCopyFileServerSide() throws Exception {
        ...
        assertNotEquals(status.getId(), new StoregateCopyFeature(session, nodeid).copy(test, copy, new TransferStatus(), new DisabledConnectionCallback()).attributes().getFileId());
    ...
    public void testCopyServerSideToExistingFile() throws Exception {
        ...
        assertTrue(feature.isSupported(test, copy));
        assertNotEquals(status.getId(), new StoregateCopyFeature(session, fileid).copy(test, copy, new TransferStatus().exists(true), new DisabledConnectionCallback()).attributes().getFileId());
    ...
    public void testCopyWithRenameToExistingFile() throws Exception {
        ...
        assertNotEquals(status.getId(), new StoregateCopyFeature(session, fileid).copy(test, test2, new TransferStatus().exists(true), new DisabledConnectionCallback()).attributes().getFileId());
        final Find find = new DefaultFindFeature(session);
    ...
    public void testCopyDirectoryServerSide() throws Exception {
        ...
        assertNotEquals(status.getId(), copy.attributes().getFileId());
