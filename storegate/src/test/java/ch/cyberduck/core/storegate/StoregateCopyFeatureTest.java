...
public class StoregateCopyFeatureTest extends AbstractStoregateTest {
    ...
    public void testCopyFileWithRename() throws Exception {
        ...
        final Path room = new StoregateDirectoryFeature(session, nodeid).mkdir(
            new Path(new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory, Path.Type.volume, Path.Type.triplecrypt)), null, new TransferStatus());
        final Path test = new StoregateTouchFeature(session, nodeid).touch(new Path(room, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.file)), new TransferStatus());
        final Path copy = new Path(new StoregateDirectoryFeature(session, nodeid).mkdir(new Path(room, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), null, new TransferStatus()), new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.file));
        final StoregateCopyFeature feature = new StoregateCopyFeature(session, nodeid);
        assertNotNull(new StoregateCopyFeature(session, nodeid).copy(test, copy, new TransferStatus(), new DisabledConnectionCallback()).attributes().getVersionId());
        assertTrue(new DefaultFindFeature(session).find(test));
        assertTrue(new DefaultFindFeature(session).find(copy));
    ...
    public void testCopyServerSideToExistingFile() throws Exception {
        final StoregateIdProvider nodeid = new StoregateIdProvider(session).withCache(cache);
        final Path top = new StoregateDirectoryFeature(session, nodeid).mkdir(
            new Path(new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory, Path.Type.volume, Path.Type.triplecrypt)), null, new TransferStatus());
        ...
        new StoregateDirectoryFeature(session, nodeid).mkdir(sourceFolder, null, new TransferStatus());
        new StoregateDirectoryFeature(session, nodeid).mkdir(targetFolder, null, new TransferStatus());
        ...
        final StoregateCopyFeature feature = new StoregateCopyFeature(session, nodeid);
        assertTrue(feature.isSupported(test, copy));
        assertNotNull(new StoregateCopyFeature(session, nodeid).copy(test, copy, new TransferStatus().exists(true), new DisabledConnectionCallback()).attributes().getVersionId());
        final Find find = new DefaultFindFeature(session);
        final AttributedList<Path> files = new StoregateListService(session).list(targetFolder, new DisabledListProgressListener());
        assertTrue(find.find(copy));
        new StoregateDeleteFeature(session, nodeid).delete(Collections.singletonList(top), new DisabledLoginCallback(), new Delete.DisabledCallback());
    }
    ...
    public void testCopyWithRenameToExistingFile() throws Exception {
        ...
        new StoregateDirectoryFeature(session, nodeid).mkdir(folder, null, new TransferStatus());
        ...
        assertNotNull(new StoregateCopyFeature(session, nodeid).copy(test, copy, new TransferStatus().exists(true), new DisabledConnectionCallback()).attributes().getVersionId());
        final Find find = new DefaultFindFeature(session);
        final AttributedList<Path> files = new StoregateListService(session).list(folder, new DisabledListProgressListener());
        assertTrue(find.find(copy));
        new StoregateDeleteFeature(session, nodeid).delete(Collections.singletonList(top), new DisabledLoginCallback(), new Delete.DisabledCallback());
    }
    ...
    public void testCopyDirectoryServerSide() throws Exception {
        ...
        final Path top = new StoregateDirectoryFeature(session, nodeid).mkdir(
            new Path(new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory, Path.Type.volume, Path.Type.triplecrypt)), null, new TransferStatus());
        final Path directory = new StoregateDirectoryFeature(session, nodeid).mkdir(new Path(top, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), null, new TransferStatus());
        final String name = new AlphanumericRandomStringService().random();
        final Path file = new StoregateTouchFeature(session, nodeid).touch(new Path(directory, name, EnumSet.of(Path.Type.file)), new TransferStatus());
        final Path target_parent = new StoregateDirectoryFeature(session, nodeid).mkdir(new Path(top, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), null, new TransferStatus());
        ...
        final Path copy = new StoregateCopyFeature(session, nodeid).copy(directory, target, new TransferStatus(), new DisabledConnectionCallback());
