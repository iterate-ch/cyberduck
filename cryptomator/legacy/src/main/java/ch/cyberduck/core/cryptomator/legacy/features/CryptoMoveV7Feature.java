...
public class CryptoMoveV7Feature implements Move {
    ...
    public CryptoMoveV7Feature(final Session<?> session, final Move delegate, final CryptomatorVault cryptomator) {
    ...
    public Path move(final Path file, final Path renamed, final TransferStatus status, final Delete.Callback callback, final ConnectionCallback connectionCallback) throws BackgroundException {
        ...
        final Path sourceEncrypted = cryptomator.encrypt(session, file, file.isDirectory());
        final Path targetEncrypted = cryptomator.encrypt(session, renamed, file.isDirectory());
