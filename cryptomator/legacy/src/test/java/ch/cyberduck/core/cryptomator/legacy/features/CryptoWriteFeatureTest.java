...
public class CryptoWriteFeatureTest {
    ...
    public void testCiphertextSize_CTR() throws Exception {
        ...
        vault.create(session, null, new VaultCredentials("test"), CryptomatorVault.VAULT_VERSION_DEPRECATED);
    ...
    public void testCiphertextSize_GCM() throws Exception {
        ...
        final CryptomatorVault vault = new CryptomatorVault(home);
        vault.create(session, null, new VaultCredentials("test"), CryptomatorVault.VAULT_VERSION);
        int headerSize = vault.getFileHeaderCryptor().headerSize();
        // zero file size
        assertEquals(headerSize, vault.toCiphertextSize(0L, 0));
        // one-byte file
