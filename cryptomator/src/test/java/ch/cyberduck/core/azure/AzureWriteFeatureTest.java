...
public class AzureWriteFeatureTest {
    ...
    public void testWrite() throws Exception {
        ...
        final OutputStream out = new CryptoWriteFeature<Void>(session, new AzureWriteFeature(session, null), cryptomator).write(test, status);
        assertNotNull(out);
        ...
        assertEquals(content.length, new CryptoWriteFeature<Void>(session, new AzureWriteFeature(session, null), cryptomator).append(test, status.getLength(), PathCache.empty()).size, 0L);
        final ByteArrayOutputStream buffer = new ByteArrayOutputStream(content.length);
        final InputStream in = new CryptoReadFeature(session, new AzureReadFeature(session, null), cryptomator).read(test, new TransferStatus().length(content.length));
        new StreamCopier(status, status).transfer(in, buffer);
        assertArrayEquals(content, buffer.toByteArray());
        new CryptoDeleteFeature(session, new AzureDeleteFeature(session, null), cryptomator).delete(Collections.<Path>singletonList(test), new DisabledLoginCallback(), new Delete.DisabledCallback());
        session.close();
    }
}
