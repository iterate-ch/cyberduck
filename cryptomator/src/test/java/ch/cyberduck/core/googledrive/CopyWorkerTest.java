...
public class CopyWorkerTest {
    ...
    public void testCopyFolder() throws Exception {
        ...
        assertTrue(new CryptoFindFeature(session, new DriveFindFeature(session), cryptomator).find(file));
        assertTrue(new CryptoFindFeature(session, new DriveFindFeature(session), cryptomator).find(fileRenamed));
        // copy folder
        ...
        assertTrue(new CryptoFindFeature(session, new DriveFindFeature(session), cryptomator).find(folder));
        assertTrue(new CryptoFindFeature(session, new DriveFindFeature(session), cryptomator).find(folderRenamed));
    ...
    public void testCopyFileIntoVault() throws Exception {
        ...
        assertTrue(new DriveFindFeature(session).find(cleartextFile));
        ...
        worker.run(session);
        assertTrue(new DriveFindFeature(session).find(cleartextFile));
        assertTrue(new CryptoFindFeature(session, new DriveFindFeature(session), cryptomator).find(encryptedFile));
        registry.clear();
        new DeleteWorker(new DisabledLoginCallback(), Collections.singletonList(vault), PathCache.empty(), new DisabledProgressListener()).run(session);
        session.close();
    }
    ...
    public void testCopyDirectoryIntoVault() throws Exception {
        ...
        new DriveTouchFeature(session).touch(cleartextFile, new TransferStatus());
        assertTrue(new DriveFindFeature(session).find(cleartextFolder));
        assertTrue(new DriveFindFeature(session).find(cleartextFile));
        final CryptoVault cryptomator = new CryptoVault(vault, new DisabledPasswordStore());
        cryptomator.create(session, null, new VaultCredentials("test"));
        final DefaultVaultRegistry registry = new DefaultVaultRegistry(new DisabledPasswordStore(), new DisabledPasswordCallback(), cryptomator);
    ...
    public void testCopyDirectoryOutsideVault() throws Exception {
        ...
        worker.run(session);
        assertTrue(new CryptoFindFeature(session, new DriveFindFeature(session), cryptomator).find(encryptedFolder));
        assertTrue(new CryptoFindFeature(session, new DriveFindFeature(session), cryptomator).find(encryptedFile));
        assertTrue(new DriveFindFeature(session).find(cleartextFolder));
        final Path fileRenamed = new Path(cleartextFolder, encryptedFile.getName(), EnumSet.of(Path.Type.file));
        assertTrue(new DriveFindFeature(session).find(fileRenamed));
        registry.clear();
