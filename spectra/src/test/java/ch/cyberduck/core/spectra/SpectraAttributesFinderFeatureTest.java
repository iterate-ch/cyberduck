...
public class SpectraAttributesFinderFeatureTest extends AbstractSpectraTest {
    ...
    public void testFindPlaceholder() throws Exception {
        final Path container = new SpectraDirectoryFeature(session).mkdir(
                new SpectraWriteFeature(session), new Path(new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory, Path.Type.volume)), new TransferStatus());
        final Path test = new SpectraDirectoryFeature(session).mkdir(new SpectraWriteFeature(session), new Path(container, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), new TransferStatus());
