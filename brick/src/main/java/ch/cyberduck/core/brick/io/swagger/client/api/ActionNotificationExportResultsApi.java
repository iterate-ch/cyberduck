...
public class ActionNotificationExportResultsApi {
  ...
  public List<ActionNotificationExportResultEntity> getActionNotificationExportResults(Integer actionNotificationExportId, Integer userId, String cursor, Integer perPage) throws ApiException {
    return getActionNotificationExportResultsWithHttpInfo(actionNotificationExportId, userId, cursor, perPage).getData();
      }
   ...
   * @param cursor Used for pagination.  Send a cursor value to resume an existing list from the point at which you left off.  Get a cursor from an existing list via the X-Files-Cursor-Next header. (optional)
   * @param perPage Number of records to show per page.  (Max: 10,000, 1,000 or less is recommended). (optional)
   * @return ApiResponse&lt;List&lt;ActionNotificationExportResultEntity&gt;&gt;
   * @throws ApiException if fails to make API call
   */
  public ApiResponse<List<ActionNotificationExportResultEntity>> getActionNotificationExportResultsWithHttpInfo(Integer actionNotificationExportId, Integer userId, String cursor, Integer perPage) throws ApiException {
