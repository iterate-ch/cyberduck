...
public class TripleCryptReadFeature implements Read {
    ...
    public TripleCryptReadFeature(final SDSSession session, final Read proxy) {
    ...
    public InputStream read(final Path file, final TransferStatus status, final ConnectionCallback connectionCallback, final PasswordCallback passwordCallback) throws BackgroundException {
        try {
            ...
            return new CryptoInputStream(proxy.read(file, status, connectionCallback, new DisabledPasswordCallback()), new FileDecryptionCipher(plainFileKey), status.getLength() + status.getOffset());
        }
