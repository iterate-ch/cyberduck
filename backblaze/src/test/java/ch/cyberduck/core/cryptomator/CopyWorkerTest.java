...
public class CopyWorkerTest extends AbstractB2Test {
    ...
    public void testCopyToDifferentFolderCryptomator() throws Exception {
        ...
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(source));
        cryptomator.getFeature(session, Directory.class, new B2DirectoryFeature(session, fileid)).mkdir(new B2WriteFeature(session, fileid), targetFolder, new TransferStatus());
    ...
    public void testCopyFolder() throws Exception {
        ...
        session.withRegistry(registry);
        final B2VersionIdProvider fileid = new B2VersionIdProvider(session);
        cryptomator.getFeature(session, Directory.class, new B2DirectoryFeature(session, fileid)).mkdir(new B2WriteFeature(session, fileid), folder, new TransferStatus());
    ...
    public void testCopyFileIntoVault() throws Exception {
        ...
        final DefaultVaultRegistry registry = new DefaultVaultRegistry(new DisabledPasswordCallback(), cryptomator);
        session.withRegistry(registry);
        cryptomator.getFeature(session, Directory.class, new B2DirectoryFeature(session, fileid)).mkdir(new B2WriteFeature(session, fileid), encryptedFolder, new TransferStatus());
