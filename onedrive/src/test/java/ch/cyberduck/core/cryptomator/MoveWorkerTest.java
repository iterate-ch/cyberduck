...
public class MoveWorkerTest extends AbstractOneDriveTest {
    ...
    public void testMoveFileOutsideVault() throws Exception {
        ...
        assertTrue(new CryptoFindFeature(session, new DefaultFindFeature(session), cryptomator).find(encryptedFile, new DisabledListProgressListener()));
        assertEquals(0L, new CryptoAttributesFeature(session, new OneDriveAttributesFinderFeature(session), cryptomator).find(encryptedFile, new DisabledListProgressListener()).getSize());
