...
public class OneDriveListServiceTest extends AbstractOneDriveTest {
    ...
    public void testListCryptomator() throws Exception {
        ...
        new CryptoTouchFeature<Void>(session, new DefaultTouchFeature<Void>(new DefaultUploadFeature<>(new OneDriveWriteFeature(session)),
            new OneDriveAttributesFinderFeature(session)), new OneDriveWriteFeature(session), cryptomator).touch(test, new TransferStatus());
