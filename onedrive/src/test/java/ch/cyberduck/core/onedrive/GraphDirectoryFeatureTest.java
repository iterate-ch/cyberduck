...
public class GraphDirectoryFeatureTest extends AbstractOneDriveTest {
    ...
    public void testMkdir() throws Exception {
        final Path target = new GraphDirectoryFeature(session).mkdir(new Path(new DefaultHomeFinderService(session).find(), new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), null, null);
    ...
    public void testWhitespaceMkdir() throws Exception {
        ...
        final Path target = new GraphDirectoryFeature(session).mkdir(new Path(new DefaultHomeFinderService(session).find(), name, EnumSet.of(Path.Type.directory)), null, null);
        assertEquals(name, target.getName());
        final AttributedList<Path> list = new GraphItemListService(session).list(new DefaultHomeFinderService(session).find(), new DisabledListProgressListener());
