...
/*
 *  ch.cyberduck.connection.Path.java
 *  Cyberduck
 *
 *  $Header$
 *  $Revision$
 ...
 * @version $Id$
 */
public class Path implements Serializable, Transferable {
    ...
    public static final DataFlavor pathFlavor
        = new DataFlavor(ch.cyberduck.connection.Path.class, "ch.cyberduck.connection.Path");
    ...
    public Path(String path, String name) {
        //Cyberduck.DEBUG("[Path] Path(" + path+","+name+")");
    ...
    private void init(String pathname) {
        //Cyberduck.DEBUG("[Path] init(" + pathname + ")");
...
//        Cyberduck.DEBUG("Path:" + this.path + ", Name:" + this.getName() + ", Parent:" + this.getParent());
    }
    ...
    public void setPath(String p) {
        Cyberduck.DEBUG("[Path] setPath(" + p + ")");
    ...
    /**
     * @return true
     * even if the file doesn't exist on the local filesystem
    ...
    /**
     * @return true
     * even if the directory doesn't exist on the local filesystem
     * but seems to be a directory because it ends with '/'
     */
    public boolean isDirectory() {
//        Cyberduck.DEBUG("[Path] isDirectory()");
        String path = this.toString();
        if(path.lastIndexOf('/') == path.length() - 1) {
            return true;
        }
    ...
    */
    public Path getRelativePath(Path relative) {
    	int index = this.getPath().indexOf(relative.getPath());
    	if(index == -1) {
    		throw new IllegalArgumentException("The argument must be part of this path");
    	}
    	else {
    		return new Path(this.getPath().substring(index + relative.getPath().length()));
    	}
    }
...
//        Cyberduck.DEBUG("[Path] getPathFragment(" + depth + ")");
