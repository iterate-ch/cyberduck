...
public final class KeychainStore extends KeyStoreSpi {
     ...
     *
     * @throws SecurityException if fails to verify
     *                           its own integrity
     */
    ...
    public Certificate engineGetCertificate(String alias) {
        ...
        if(entry != null) {
            if(entry instanceof TrustedCertEntry) {
                ...
                if(((KeyEntry) entry).chain.length == 0) {
                    return null;
                }
    ...
    public String engineGetCertificateAlias(Certificate cert) {
        ...
        for(Enumeration e = entries.keys(); e.hasMoreElements(); ) {
            ...
            else if(((KeyEntry) entry).chain.length == 0) {
                continue;
            }
            else {
    ...
    private void createTrustedCertEntry(String alias, long keychainItemRef, long creationDate, byte[] derStream) {
        ...
        try {
            ...
            log.error("KeychainStore Ignored Exception: " + e);
        }
    }
    ...
    private byte[] extractKeyData(DerInputStream stream)
         ...
         */
        for(DerValue safeBag : safeBags) {
            ObjectIdentifier bagId;
            DerInputStream sbi;
            DerValue bagValue;
            Object bagItem = null;

            sbi = safeBag.toDerInputStream();
            bagId = sbi.getOID();
            ...
            if(bagId.equals(PKCS8ShroudedKeyBag_OID)) {
                ...
                log.warn("Unsupported bag type '" + bagId + "'");
            }
        }
