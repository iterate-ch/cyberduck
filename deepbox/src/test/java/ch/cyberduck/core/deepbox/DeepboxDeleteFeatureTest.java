...
public class DeepboxDeleteFeatureTest extends AbstractDeepboxTest {
    ...
    @Test
    public void testDeleteFolderWithContent() throws Exception {
        final DeepboxIdProvider nodeid = new DeepboxIdProvider(session);
        final Path room = new DeepboxDirectoryFeature(session, nodeid).mkdir(new Path(auditing, String.format(new AlphanumericRandomStringService().random()), EnumSet.of(Path.Type.directory, Path.Type.volume)), new TransferStatus());
        final Path subfolderWithContent = new DeepboxDirectoryFeature(session, nodeid).mkdir(new Path(room, new AlphanumericRandomStringService().random().toLowerCase(), EnumSet.of(Path.Type.directory)), new TransferStatus());
        ...
        new DeepboxTouchFeature(session, nodeid).touch(file, new TransferStatus());
        assertTrue(new DefaultFindFeature(session).find(file));
        new DeepboxDeleteFeature(session, nodeid).delete(Collections.singletonList(subfolderWithContent), new DisabledLoginCallback(), new Delete.DisabledCallback());
