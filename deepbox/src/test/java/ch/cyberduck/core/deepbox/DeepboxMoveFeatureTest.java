...
public class DeepboxMoveFeatureTest extends AbstractDeepboxTest {
    ...
    public void testMove() throws Exception {
        ...
        final Path moved = new DeepboxMoveFeature(session, fileid).move(test,
                new Path(documents, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.file)), new TransferStatus(), new Delete.DisabledCallback(), ConnectionCallback.noop);
        ...
        new DeepboxDeleteFeature(session, fileid).delete(Collections.singletonList(moved), LoginCallback.noop, new Delete.DisabledCallback());
    }
    ...
    public void testMoveDirectory() throws Exception {
        final DeepboxIdProvider fileid = new DeepboxIdProvider(session);
        final Path documents = new Path("/ORG 4 - DeepBox Desktop App/ORG 4 - DeepBox Desktop App/ORG3:Box1/Documents/", EnumSet.of(Path.Type.directory, Path.Type.volume));
        final Path test = new DeepboxDirectoryFeature(session, fileid).mkdir(new DeepboxWriteFeature(session, fileid), new Path(documents, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), new TransferStatus());
        final String sourceId = test.attributes().getFileId();
        assertNotNull(sourceId);
        new DeepboxTouchFeature(session, fileid).touch(new DeepboxWriteFeature(session, fileid), new Path(test, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.file)), new TransferStatus());
        final Path target = new Path(documents, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory));
        final Path moved = new DeepboxMoveFeature(session, fileid).move(test, target, new TransferStatus(), new Delete.DisabledCallback(), ConnectionCallback.noop);
        assertEquals(sourceId, moved.attributes().getFileId());
    ...
    public void testMoveOverrideFile() throws Exception {
        ...
        {
            final byte[] content = RandomUtils.nextBytes(124);
            new StreamCopier(StreamCancelation.noop, StreamProgress.noop).transfer(new ByteArrayInputStream(content),
                    new DeepboxWriteFeature(session, fileid).write(test, new TransferStatus().setExists(true), ConnectionCallback.noop));
        ...
        final Path target = new DeepboxTouchFeature(session, fileid).touch(
                new DeepboxWriteFeature(session, fileid), new Path(documents, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.file)), new TransferStatus());
        {
            final byte[] content = RandomUtils.nextBytes(165);
            new StreamCopier(StreamCancelation.noop, StreamProgress.noop).transfer(new ByteArrayInputStream(content),
                    new DeepboxWriteFeature(session, fileid).write(target, new TransferStatus().setExists(true), ConnectionCallback.noop));
        ...
        assertNotEquals(originalTestAttributes.getModificationDate(), originalTargetAttributes.getModificationDate());
        assertNotEquals(originalTestAttributes, originalTargetAttributes);

        final String sourceFileId = originalTestAttributes.getFileId();
        assertNotNull(sourceFileId);
        final Path renamed = new DeepboxMoveFeature(session, fileid).move(test, target,
                new TransferStatus().setExists(true), new Delete.DisabledCallback(), ConnectionCallback.noop);
        assertEquals(sourceFileId, renamed.attributes().getFileId());
        ...
        final PathAttributes overriddenTargetAttributes = new DeepboxAttributesFinderFeature(session, fileid).find(target);
        assertEquals(renamed.attributes(), overriddenTargetAttributes);
        assertNotEquals(originalTargetAttributes.getFileId(), overriddenTargetAttributes.getFileId());
        assertNotEquals(originalTargetAttributes.getModificationDate(), overriddenTargetAttributes.getModificationDate());
        assertEquals(sourceFileId, overriddenTargetAttributes.getFileId());
        ...
        assertEquals(originalTestAttributes.getSize(), overriddenTargetAttributes.getSize());

        final PathAttributes trashedTargetAttributes = new DeepboxAttributesFinderFeature(session, fileid).find(targetInTrash);
    ...
    public void testMoveOverrideFolder() throws Exception {
        ...
        final Path test = new DeepboxDirectoryFeature(session, fileid).mkdir(
                new DeepboxWriteFeature(session, fileid), new Path(documents, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), new TransferStatus());
        ...
        new DeepboxMoveFeature(session, fileid).move(test, target, new TransferStatus().setExists(true), new Delete.DisabledCallback(), ConnectionCallback.noop);
        assertFalse(new DeepboxFindFeature(session, fileid).find(test.withAttributes(PathAttributes.EMPTY)));
        assertTrue(new DeepboxFindFeature(session, fileid).find(target.withAttributes(PathAttributes.EMPTY)));
        assertTrue(new DeepboxFindFeature(session, fileid).find(targetInTrash));

        final PathAttributes overriddenTargetAttributes = new DeepboxAttributesFinderFeature(session, fileid).find(target.withAttributes(PathAttributes.EMPTY));
        ...
        final PathAttributes trashedTargetAttributes = new DeepboxAttributesFinderFeature(session, fileid).find(targetInTrash.withAttributes(PathAttributes.EMPTY));
        assertNotNull(originalTargetAttributes.getFileId());
        assertEquals(originalTargetAttributes.getFileId(), trashedTargetAttributes.getFileId());
        assertEquals(originalTargetAttributes.getModificationDate(), trashedTargetAttributes.getModificationDate());
        assertEquals(originalTargetAttributes.getChecksum(), trashedTargetAttributes.getChecksum());
    ...
    public void testMoveNotFound() {
        ...
        assertThrows(NotfoundException.class, () -> new DeepboxMoveFeature(session, fileid).move(test, renamed, new TransferStatus(), new Delete.DisabledCallback(), ConnectionCallback.noop));
    }
    ...
    public void testNoMoveRenameInbox() throws Exception {
        final DeepboxIdProvider nodeid = new DeepboxIdProvider(session);
        final Path inbox = new Path("/ORG 1 - DeepBox Desktop App/ORG 1 - DeepBox Desktop App/ORG1:Box1/Inbox", EnumSet.of(Path.Type.directory, Path.Type.volume));
    ...
    public void testNoMoveRenameDocuments() throws Exception {
        final DeepboxIdProvider nodeid = new DeepboxIdProvider(session);
        final Path documents = new Path("/ORG 1 - DeepBox Desktop App/ORG 1 - DeepBox Desktop App/ORG1:Box1/Documents", EnumSet.of(Path.Type.directory, Path.Type.volume));
    ...
    public void testNoMoveRenameFileFromTrash() throws Exception {
        final DeepboxIdProvider nodeid = new DeepboxIdProvider(session);
        final Path documents = new Path("/ORG 4 - DeepBox Desktop App/ORG 4 - DeepBox Desktop App/ORG3:Box1/Documents", EnumSet.of(Path.Type.directory, Path.Type.volume));
        final Path file = new DeepboxTouchFeature(session, nodeid).touch(new DeepboxWriteFeature(session, nodeid), new Path(documents, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.file)), new TransferStatus());
        ...
        new DeepboxDeleteFeature(session, nodeid).delete(Collections.singletonList(fileInTrash), LoginCallback.noop, new Delete.DisabledCallback());
    }
    ...
    public void testNoMoveRenameFileAndFolderDocuments() throws Exception {
        final DeepboxIdProvider nodeid = new DeepboxIdProvider(session);
        final Path folder = new Path("/ORG 1 - DeepBox Desktop App/ORG 1 - DeepBox Desktop App/ORG1:Box1/Documents/Invoices : Receipts", EnumSet.of(Path.Type.directory));
