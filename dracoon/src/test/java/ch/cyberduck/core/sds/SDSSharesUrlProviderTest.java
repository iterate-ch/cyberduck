...
public class SDSSharesUrlProviderTest extends AbstractSDSTest {
    ...
    public void testUploadAccountEncrypted() throws Exception {
        final Path room = new Path("test", EnumSet.of(Path.Type.directory, Path.Type.volume, Path.Type.triplecrypt));
        final SDSNodeIdProvider nodeid = new SDSNodeIdProvider(session);
        final Path folder = new SDSDirectoryFeature(session, nodeid).mkdir(new Path(room, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory, Path.Type.triplecrypt)), new TransferStatus());
