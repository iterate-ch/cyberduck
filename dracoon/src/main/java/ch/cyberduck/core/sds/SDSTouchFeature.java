...
public class SDSTouchFeature extends DefaultTouchFeature<Node> {
    ...
    public SDSTouchFeature(final SDSSession session, final SDSNodeIdProvider nodeid) {
        super(new SDSDelegatingWriteFeature(session, nodeid, session.isS3DirectUploadSupported() ? new SDSDirectS3MultipartWriteFeature(session, nodeid) : new SDSMultipartWriteFeature(session, nodeid)));
