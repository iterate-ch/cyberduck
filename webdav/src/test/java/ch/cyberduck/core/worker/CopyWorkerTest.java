...
public class CopyWorkerTest extends AbstractDAVTest {
    ...
    public void testCopyFileToDirectory() throws Exception {
        ...
        new DAVTouchFeature(session).touch(new DAVWriteFeature(session), sourceFile, new TransferStatus());
    ...
    public void testCopyDirectory() throws Exception {
        final Path home = new DefaultHomeFinderService(session).find();
        final Path folder = new DAVDirectoryFeature(session).mkdir(new DAVWriteFeature(session), new Path(home, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.directory)), new TransferStatus());
        final Path sourceFile = new DAVTouchFeature(session).touch(new DAVWriteFeature(session), new Path(folder, new AlphanumericRandomStringService().random(), EnumSet.of(Path.Type.file)), new TransferStatus());
