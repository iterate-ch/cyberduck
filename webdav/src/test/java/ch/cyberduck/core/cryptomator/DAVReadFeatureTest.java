...
public class DAVReadFeatureTest extends AbstractDAVTest {
    ...
    public void testReadRange() throws Exception {
        ...
        assertEquals(content.length, new CryptoUploadFeature<>(session, new DAVUploadFeature(), cryptomator).append(test, status.setRemote(cryptomator.getFeature(session, AttributesFinder.class, new DAVAttributesFinderFeature(session)).find(test))).offset, 0L);
        {
