...
public class CopyWorkerTest extends AbstractDAVTest {
    ...
    public void testCopyToDifferentFolderCryptomator() throws Exception {
        ...
        new CryptoTouchFeature<>(session, new DefaultTouchFeature<Void>(
                session), cryptomator).touch(new DAVWriteFeature(session), source, new TransferStatus());
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(source));
        cryptomator.getFeature(session, Directory.class, new DAVDirectoryFeature(session)).mkdir(new DAVWriteFeature(session), targetFolder, new TransferStatus());
    ...
    public void testCopyFolder() throws Exception {
        ...
        final DefaultVaultRegistry registry = new DefaultVaultRegistry(new DisabledPasswordCallback(), cryptomator);
        session.withRegistry(registry);
        cryptomator.getFeature(session, Directory.class, new DAVDirectoryFeature(session)).mkdir(new DAVWriteFeature(session), folder, new TransferStatus());
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(folder));
        new CryptoTouchFeature<>(session, new DefaultTouchFeature<Void>(
                session), cryptomator).touch(new DAVWriteFeature(session), file, new TransferStatus());
    ...
    public void testCopyFileIntoVault() throws Exception {
        ...
        final DefaultVaultRegistry registry = new DefaultVaultRegistry(new DisabledPasswordCallback(), cryptomator);
        session.withRegistry(registry);
        cryptomator.getFeature(session, Directory.class, new DAVDirectoryFeature(session)).mkdir(new DAVWriteFeature(session), encryptedFolder, new TransferStatus());
    ...
    public void testCopyDirectoryOutsideVault() throws Exception {
        ...
        cryptomator.getFeature(session, Directory.class, new DAVDirectoryFeature(session)).mkdir(new DAVWriteFeature(session), encryptedFolder, new TransferStatus());
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(encryptedFolder));
        new CryptoTouchFeature<>(session, new DefaultTouchFeature<Void>(session), cryptomator).touch(new DAVWriteFeature(session), encryptedFile, new TransferStatus());
        assertTrue(cryptomator.getFeature(session, Find.class, new DefaultFindFeature(session)).find(encryptedFile));
