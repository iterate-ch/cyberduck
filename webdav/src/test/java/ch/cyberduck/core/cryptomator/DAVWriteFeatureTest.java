...
public class DAVWriteFeatureTest extends AbstractDAVTest {
    ...
    public void testWrite() throws Exception {
        ...
        assertEquals(content.length, new CryptoListService(session, new DAVListService(session), new DAVFindFeature(session), cryptomator).list(test.getParent(), new DisabledListProgressListener()).get(test).attributes().getSize());
        assertEquals(content.length, new CryptoWriteFeature<>(session, new DAVWriteFeature(session, true), cryptomator).append(test, status
            .withRemote(new CryptoAttributesFeature(session, new DAVAttributesFinderFeature(session), cryptomator).find(test))).size, 0L);
