...
public class GoogleStorageTouchFeatureTest extends AbstractGoogleStorageTest {
    ...
    public void testTouch() throws Exception {
        ...
        final Path test = new GoogleStorageTouchFeature(session).touch(
                new Path(bucket, String.format("%s %s", new AlphanumericRandomStringService().random(), new AlphanumericRandomStringService().random()), EnumSet.of(Path.Type.file)), new TransferStatus().setMime("text/plain"));
